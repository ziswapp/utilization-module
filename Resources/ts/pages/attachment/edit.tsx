...
import { EditAttachmentListCard } from "../../components/edit-attachment-list";
...
const Page = ({ utilization, attachments }: PageProps) => {
  ...
  const onDeleteItem = (file: Partial<Ziswapp.Domain.Foundation.Model.File>) => {
    router.delete(`/utilization/${utilization.id}/utilization/${file.uuid}`, {
      preserveScroll: true,
      onSuccess: () => {
        toast({
          title: "Success",
